...
function init() {
  ...
  function parseSections() {
    var headlines = document.querySelectorAll('main > section > h1, main > section > h2, footer > section > h1');
    var lastSection;
  ...
  // document.querySelector('.scroll-first a').addEventListener('click', function (e) {
  //   e.preventDefault();
  //   smoothScrollToStart();
  // });
  ...
  form.addEventListener('submit', function(e) {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        alert('お問い合わせありがとうございました。お問い合わせへの回答に関しまして、近日中にメールにてご回答いたします。');
        location.reload();
      }
    }

    xhr.addEventListener('error', function(e) {
      alert('送信に失敗しました。再度にご確認お願い致します。');
    });
